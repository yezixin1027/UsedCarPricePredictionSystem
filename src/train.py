...
def generate_comparison_table():
    """返回算法核心假设与适用条件对比表 (含新增模型)。"""
    table = [
        {
            '算法': 'Ridge (岭回归)',
            '模型类型': '线性模型 (L2 正则化)',
            '核心假设': '特征与目标线性相关；误差 i.i.d. 且同方差',
            '优势': '训练极快、可解释性强、不易过拟合',
            '劣势': '无法捕获非线性；对异常值敏感',
        ...
        {
            '算法': 'LightGBM',
            '模型类型': '梯度提升树 (Boosting, 直方图算法)',
            '核心假设': '无严格分布假设；leaf-wise 生长效率高',
            '优势': '训练极快；内存效率高；原生支持类别特征；精度高',
            '劣势': '小数据易过拟合；leaf-wise 可能不稳定',
            '适用场景': '大规模数据集；特征维度高；需要快速迭代的生产环境'
        },
        {
            '算法': 'XGBoost',
            '模型类型': '梯度提升树 (Boosting, 预排序)',
            '核心假设': '无严格分布假设；正则化完善',
            '优势': 'L1/L2 正则化完善；社区成熟；内置 CV',
            '劣势': '预排序内存开销大；比 LightGBM 慢',
            '适用场景': '对稳健性要求高的竞赛/生产场景'
        },
        {
            '算法': 'CatBoost',
            '模型类型': '梯度提升树 (Boosting, 有序提升)',
            '核心假设': '有序提升减少数据泄露；原生类别处理',
            '优势': '原生类别处理 (无需手动编码)；默认参数即好；有序提升',
            '劣势': '训练较慢；模型体积大',
            '适用场景': '大量类别特征；不想做繁琐编码的快速原型'
        },
        {
            '算法': 'Stacking (集成融合)',
            '模型类型': '异质集成 (LGB+XGB+Cat → ElasticNet)',
            '核心假设': '不同算法的误差不相关，可互补',
            '优势': '融合多种算法优势；通常优于单模型；稳健性强',
            '劣势': '训练时间长；模型体积大；可解释性差',
            '适用场景': '追求极致精度；生产环境中对精度要求最高的场景'
        }
    ]
